...
def replace_word_in_list(all_words:list[str], good_word:str, bad_word:str) -> list[str]:
...
def return_multiple_ids(mapping: dict, to_convert: tuple[tuple[str]]) -> tuple[tuple[int]]:
...
def populate_court_case(conn: connection, ids: tuple[tuple[str]], summaries: tuple[tuple[str]],
    v_ids: tuple[tuple[int]], titles: tuple[tuple[str]], dates: tuple[tuple[str]],
    numbers: tuple[tuple[str]], urls: tuple[tuple[str]], c_ids: tuple[tuple[str]],
    v_summmaries: tuple[tuple[str]]):
    # pylint: disable=R0913
    """Add data to the court_case table to be able to then obtain all ids"""
    matched = []
    for i, value in enumerate(ids):
        matched.append((value, summaries[i], v_ids[i], titles[i], dates[i], numbers[i], urls[i],
                        c_ids[i],v_summmaries[i]))
    query = """
...
def populate_lawyer(conn: connection, all_lawyers: list[str], all_law_firm_ids: list[int]):
...
def populate_participant_assignment(conn: connection, case_id: int, all_participant_ids: list[int],
                                    all_lawyer_ids: list[int], all_person_type: list[bool]):
    ...
    for index, accusant in enumerate(all_participant_ids):
        matched.append((case_id, accusant, all_lawyer_ids[index], all_person_type[index]))
    query = """
...
def process_people_data(people: list[tuple[tuple[str]|str|bool]]):
...
def replace_data(unmatched_judges: list[tuple[str]],
                 matched_judges: list[tuple[str]]) -> list[tuple[str]]:
    """Updates judges' names if they weren't in the table"""
    result = []
    matched_judge_idx = 0
    for judge_tuple in unmatched_judges:
        new_tuple = tuple(matched_judges[matched_judge_idx + i][0] for i in range(len(judge_tuple)))
...
def insert_to_database(conn: connection, transformed_data: dict) -> str:
    ...
    allowed_verdicts = ('Guilty','Not Guilty','Dismissed','Acquitted','Hung Jury','Claimant Wins',
                        'Defendant Wins','Settlement','Struck Out','Appeal Dismissed',
    ...
    matched_judges_list = add_judges(conn, [(judge,) for case in judges for judge in case])
    ...
    lawyer_list, law_firm_list, people_list = process_people_data(transformed_data["people"])
    ...
    populate_court_case(conn, transformed_data["case_ids"], transformed_data["summ"], verdict_ids,
                    transformed_data["title"], transformed_data["date"], transformed_data["number"],
                    transformed_data["url"], court_ids, transformed_data["v_sum"])
    ...
    for i, case_id in enumerate(transformed_data["case_ids"]):
        ...
        populate_participant_assignment(conn, case_id, participant_ids, lawyer_ids, lawyer_list[0])
...
if __name__ == "__main__":
    ...
    transform = {
        ...
        "summ": ["""This case involved a judicial review application by the Stamford Chamber
        ...
        "url": ["https://caselaw.nationalarchives.gov.uk/ewhc/admin/2009/719",
            "https://caselaw.nationalarchives.gov.uk/ewca/civ/2009/309",],
        "v_sum": ["""The court dismissed the claim for judicial review, concluding that the
