...
def get_bench_judges(url: str) -> list[str]:
    ...
    return [
        ...
        if index % 2 == 1 and item.get_text() != "Name" and item.get_text()[0].isalpha()
    ]
...
def get_diversity_high_court_judges(url: str) -> list[str]:
    ...
    return [
        item.get_text()
        for index, item in enumerate(get_judge_rows(url))
        if index % 2 == 0 and item.get_text() != "Name" and item.get_text()[0].isalpha()
    ]
...
def gather_all_judges() -> list[str]:
    ...
    return (
        high_court_masters
        + bench_chairs
        + district_judges_magistrates
        + diversity_community_judges
        + judge_advocates_general
        + circuit_judges
        + district_judges
        + diversity_community_magistrates
    )
